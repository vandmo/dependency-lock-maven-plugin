...
final class LockProjectHelper {
  ...
  Plugins loadPlugins(MavenProject project) throws MojoExecutionException {
    ...
    validateProfilePlugins(project, declaredPlugins);
    return Plugins.from(result);
  }

  private void validateProfilePlugins(MavenProject project, Map<String, Plugin> declaredPlugins) {
    ...
    if (!missingPlugins.isEmpty()) {
      log.warn(
          String.format(
              ROOT,
              "%s plugin(s) were found in profiles but not declared in the current build. This"
                  + " is not recommended and may cause lock files to be inconsistent.",
              missingPlugins.size()));
      missingPlugins.keySet().stream().sorted().forEach(pluginId -> log.warn("   * " + pluginId));
    }
  }
