...
public final class LockedParents extends AbstractLockedEntries<Artifact> {
  ...
  public DiffReport compareWith(Parents actualEntities, Filters filters) {
    ...
    Iterator<Artifact> lockedEntitiesIterator = this.parents.stream().map(parent -> parent.artifact).iterator();
    Iterator<Artifact> actualEntitiesIterator = actualEntities.stream().map(parent -> parent.artifact).iterator();
